...
4. **Built-in middleware (`src/middleware/`)** are `Middleware` factories: `json`, `auth`, `cache`, `validate`, `cors`, `logger`, `errorBoundary` (barrel in `middleware/index.ts`). `cache` wraps `res.json`/`res.send` on a miss to capture only **2xx** payloads, with a bounded (max-size, TTL-evicting) store. `json` enforces a body-size `limit` and surfaces `BodyError` (413/400) from `req.json()` (see `errors.ts`). `auth` throws at construction if neither `token` nor `verify` is supplied (no silent allow-all).
...
- **`Raptur` is silent by default.** The constructor takes `number | RapturOptions` and only logs (banner, route registrations, "hunting on port") when `{ silent: false }`. Route methods cover GET/POST/PUT/PATCH/DELETE/OPTIONS/HEAD; `app.stop()` returns a promise. Examples pass `{ silent: false }` for visible output.
